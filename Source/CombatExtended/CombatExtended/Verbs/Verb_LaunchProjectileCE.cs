...
public class Verb_LaunchProjectileCE : Verb
{
    ...
    public override float EffectiveRange => base.EffectiveRange * (1f + (base.EquipmentSource?.GetStatValue(StatDefOf.RangedWeapon_RangeMultiplier) - 1f ?? 0f) + (projectilePropsCE?.effectiveRangeMultiplier - 1f ?? 0f));
    ...
    public float RecoilAmount
    {
        get
        {
            float recoil = VerbPropsCE.recoilAmount * (1f + (EquipmentSource?.GetStatValue(CE_StatDefOf.CE_RangedWeapon_RecoilMultiplier) - 1f ?? 0f) + (projectilePropsCE?.recoilMultiplier - 1f ?? 0f));
            WeaponPlatform platform = this.WeaponPlatform;
            if (platform != null)
            {
    ...
    public override float WarmupTime => base.WarmupTime * (1f + (base.EquipmentSource?.GetStatValue(StatDefOf.RangedWeapon_WarmupMultiplier) - 1f ?? 0f) + (projectilePropsCE?.warmupMultiplier - 1f ?? 0f));
