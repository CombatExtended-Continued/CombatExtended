...
namespace CombatExtended
{
    public class Apparel_VisibleAccessory : Apparel
    {
    	...
    	public override void DrawWornExtras()
        {
            ...
            if (this.onHead)
            {
            	Log.ErrorOnce(string.Concat("Combat Extended FastTrack Edition :: Apparel_VisibleAccessory: The head drawing code is incomplete and the apparel '",
       	...
       	protected bool onHead
    	{
       		get {
       			if (!_OnHeadCache.ContainsKey(def.defName))
       			{
		   			...
		   			if (!_OnHeadCache.ContainsKey(def.defName))
		   			{
		   				Log.ErrorOnce(string.Concat("Combat Extended FastTrack Edition :: ", this.GetType(), " was unable to determine if body or head on item '", Label,
