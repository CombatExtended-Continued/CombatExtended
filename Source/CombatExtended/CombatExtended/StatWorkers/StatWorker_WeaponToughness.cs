    ...
    public class StatWorker_WeaponToughness : StatWorker
    {
        ...
        public override string GetExplanationFinalizePart(StatRequest req, ToStringNumberSense numberSense, float finalVal)
        {
            ...
            if (req.Thing != null)
            {
                ...
                {
                    result += "\n" + "CE_Holder_WeaponToughnessEffect".Translate() + ((req.Thing?.TryGetComp<CompEquippable>()?.Holder.GetStatValue(CE_StatDefOf.MeleeParryChance) ?? 1f) * parryChanceFactor).ToStringPercent();
                }
            }
