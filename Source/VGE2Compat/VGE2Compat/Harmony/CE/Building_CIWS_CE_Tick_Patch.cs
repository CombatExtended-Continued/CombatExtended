...
using CombatExtended.Compatibility.VGE2Compat;
using HarmonyLib;
using RimWorld;
using Verse;
...
[HarmonyPatch(typeof(Building_CIWS_CE), nameof(Building_CIWS_CE.Tick))]
public static class Building_CIWS_CE_Tick_Patch
{
    public static void Postfix(Building_CIWS_CE __instance)
    {
        if (__instance.IsHashIntervalTick(250) && __instance.Faction == Faction.OfPlayer && __instance.def.defName == "VGE_PointDefenseTurret")
        {
