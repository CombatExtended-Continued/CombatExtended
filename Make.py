...
with XMLOpen(CSPROJ) as csproj:
    ...
    if DOWNLOAD_LIBS:
        ...
        for idx, package in enumerate(csproj.getElementsByTagName("PackageReference")):
            ...
            if os.system(f"wget https://www.nuget.org/api/v2/package/{name}/{version} -O {tdir}/downloads/rwref-{idx}.zip"):
                raise Exception(f"Can't find version {version} of {name} on nuget")
            os.system(f"unzip -o {tdir}/downloads/rwref-{idx}.zip -d {tdir}/downloads/unpack")
